...
#    nms_thesh = float(args.nms_thresh)
    ...
    def write(x, batches, results):
        ...
        start_point = (int(c1[0].numpy()), int(c1[1].numpy()))
        end_point = (int(c2[0].numpy()), int(c2[1].numpy()))
        print(label)
        # exit()
        ...
        cv2.putText(image, label, start_point, cv2.FONT_HERSHEY_PLAIN, 1, [225, 255, 255], 1)
        return image;
    ...
    q1 = q.split("\n")
    q2 = q1[0]
    det_names1 = str("./det/"+q2)

    #print(str(osp.realpath('.')))
    #print(det_names1)

    join = osp.join(osp.realpath('.'), det_names1)
    print("join")
    print(join)
    print("join")
    print(type(join))
    img_fin = cv2.imread(join)
    cv2.imshow('img fin, nms: '+str(nms_thesh)+" confidence: "+str(confidence), img_fin)
